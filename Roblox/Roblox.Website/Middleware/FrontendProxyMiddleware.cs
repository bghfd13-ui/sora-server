...
public class FrontendProxyMiddleware
{
    ...
    private async Task<HttpResponseMessage> ProxyRequestAsync(HttpContext ctx, string url)
    {
        /*
         * Local development:
         *   FRONTEND_URL is not set -> http://localhost:3000
         *
         * Render/production:
         *   FRONTEND_URL should contain the URL of the sora-frontend service,
         *   for example: https://sora-frontend-xxxx.onrender.com
         *
         * The old implementation always used localhost:3000, which works only
         * when the frontend runs inside the same machine/container.
         */
        ...
        if (!Uri.TryCreate(frontendUrl.TrimEnd('/'), UriKind.Absolute, out var frontendBase))
            throw new InvalidOperationException("Invalid FRONTEND_URL: " + frontendUrl);

        if (frontendBase.Scheme != Uri.UriSchemeHttp &&
            frontendBase.Scheme != Uri.UriSchemeHttps)
            throw new InvalidOperationException("FRONTEND_URL must use http or https: " + frontendUrl);

        // requestUrl is generated by ASP.NET and contains only the path/query.
        // Resolve it against the explicitly configured frontend base URL.
        var safeUrl = new Uri(
            frontendBase,
