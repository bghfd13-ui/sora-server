...
public class AvatarService : ServiceBase, IService {
    ...
    public async Task<AvatarWithColors> GetAvatar(long userId)
    {
        ...
        // SORA OWNER AVATAR FALLBACK
        // User 1 is the owner/default avatar.
        // If a user has no user_avatar row yet, return the owner avatar.
        if (existingAvatar == null && userId != 1)
        {
            existingAvatar = await db.QuerySingleOrDefaultAsync<DatabaseAvatarWithImages>(
                "SELECT * FROM user_avatar WHERE user_id = 1");

            if (existingAvatar != null)
            {
                Console.WriteLine("[SORA] Avatar fallback: user " + userId + " -> owner avatar (user 1)");
            }
        }
