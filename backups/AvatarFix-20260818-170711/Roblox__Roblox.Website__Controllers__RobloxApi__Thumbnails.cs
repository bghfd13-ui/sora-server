...
public class RbxThumbnails : ControllerBase
{
    ...
    public async Task<dynamic> GetAvatarThumbnailJson([Required] long userId)
    {
        ...
        var imageUrl = result.FirstOrDefault()?.imageUrl;
        return new
        {
            Url = string.IsNullOrEmpty(imageUrl) ? null : $"{Configuration.BaseUrl}{imageUrl}",
            Final = true,
            SubstitutionType = string.IsNullOrEmpty(imageUrl) ? 1 : 0
        };
    }
